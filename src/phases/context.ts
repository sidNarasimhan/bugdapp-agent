...
import { analyzeBundles, type BundleAnalysis } from '../browser/bundle.js';
import { startNetworkCapture, type NetworkCapture } from '../browser/network.js';
...
export async function crawlDApp(ctx: BrowserCtx, url: string): Promise<CrawlResult> {
  ...
  try {
    const { getProfileOrThrow } = await import('../agent/profiles/registry.js');
