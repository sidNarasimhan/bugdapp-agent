...
import { listSpecs } from './matcher.js';
...
export function listFlowsReply(): string {
  ...
  const head = `**${dapp.name}** (${dapp.url})`;
  const profileLines = [
    `- Network: ${dapp.chain.name} (chainId ${dapp.chain.id})`,
    `- Archetype: ${dapp.archetype}`,
  ].join('\n');
  const specBlock = specs.length === 0
    ? '**No cached specs on disk** — I\'ll drive the browser myself (act mode).'
    : `**Known specs (${specs.length}):**\n${specs.slice(0, 20).map((s: string) => '• `' + s + '`').join('\n')}${specs.length > 20 ? '\n…' : ''}`;
  const examples = [
    '**Try asking me:**',
    '• "open a 100x long on ETH-USD with $1 collateral (ZFP)"',
    '• "close my position"',
    '• "check if the form rejects below-minimum amounts"',
