...
export interface HealRunOptions {
  dAppUrl: string;
  outputDir: string;
  /** If true, re-run healed specs under Playwright after healing to verify. Default: true. */
...
export async function runSuiteWithHealing(opts: HealRunOptions): Promise<HealRunSummary> {
  ...
  const profile = findProfile(opts.dAppUrl) ?? avantisProfile;

  emit(`[heal-runner] Phase 1: Playwright (as-is)`);
  const r1 = await runPlaywright(opts.outputDir, emit);
  ...
  if (failures.length > 0) {
    ...
    for (const f of failures) {
      ...
      const agent = await runExecutor({ task, profile, initialUrl: profile.url });
      ...
      const specPath = join(opts.outputDir, 'tests', f.specFile);
  ...
  if (healed.length > 0 && opts.verifyAfterHeal !== false) {
    ...
    const r2 = await runPlaywright(opts.outputDir, emit, grep);
